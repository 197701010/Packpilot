...
// HULPFUNCTIE: Bepaalt of de tekstkleur wit of zwart moet zijn voor de leesbaarheid.
...
export default function MockupColorPage() {
  const router = useRouter();
  const [company, setCompany] = useState("");
  const [slogan, setSlogan] = useState("");
  const [selectedColors, setSelectedColors] = useState([]);

  // De volledige, gesorteerde colorMap
  const colorMap = {
  ...
  // NIEUWE FUNCTIE: Verzamelt alle data en stuurt door naar de /design pagina
  const handleStartMockup = () => {
    // Converteer geselecteerde kleurnamen naar hex-codes (zonder #)
    const selectedHexCodes = selectedColors.map(name => colorMap[name].replace('#', ''));
    
    // Bouw de query parameters veilig op
    ...
    // Navigeer naar de nieuwe pagina met alle data in de URL
    router.push(`/mockup/design?${params.toString()}`);
  };
