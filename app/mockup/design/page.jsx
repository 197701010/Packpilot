...
export default function DesignPage() {
  ...
  const [companyInfo, setCompanyInfo] = useState({ company: '', slogan: '', colors: [] });
  ...
  useEffect(() => {
    ...
    setCompanyInfo({ company, slogan, colors });
  }, [searchParams]);
  ...
  const handleNext = () => {
    const params = new URLSearchParams({
      ...
      colors: companyInfo.colors.join(','),
      description,
      styles: selectedStyles.join(','),
    });
    router.push(`/mockup/final?${params.toString()}`);
  };
        ...
        <header className="text-center mb-10">
          ...
          <div className="flex justify-between items-center mt-6">
            ...
            <div className="flex gap-2">
              <button onClick={handleBack} className="px-5 py-2 border border-gray-300 rounded-lg text-gray-700 font-semibold hover:bg-gray-50">Back</button>
        ...
        <div className="bg-white">
          ...
          <div className="mt-2 relative">
            <textarea
              ...
              maxLength={500}
              placeholder="e.g. Create a sleek matte box with pastel pops, shiny foil touches, and bold clear text that delights every unboxing."
              className="w-full h-36 p-4 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-purple-500 focus:outline-none"
            />
        ...
        <div className="mt-8">
            <div className="flex gap-4 border-b">
                <button 
                    ...
                    className={`flex items-center gap-2 py-3 px-4 text-gray-600 font-semibold rounded-t-lg relative ${activeTab === 'Upload' ? 'text-purple-600' : ''}`}
                >
                    {/* AANPASSING: De <svg> is vervangen door een component */}
                ...
                <button 
                    ...
                    className={`flex items-center gap-2 py-3 px-4 text-gray-600 font-semibold rounded-t-lg relative ${activeTab === 'Styles' ? 'text-purple-600' : ''}`}
                >
                    {/* AANPASSING: De <svg> is vervangen door een component */}
        ...
        <div className="mt-6">
            {activeTab === 'Upload' && (
                <div className="text-center p-10 border-2 border-dashed border-gray-300 rounded-lg">
                    <p>Hier komt de upload functionaliteit.</p>
                </div>
            )}
            {activeTab === 'Styles' && (
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {stylesData.map(style => (
                            <div
                                ...
                                className={`flex items-center gap-4 p-3 border rounded-xl cursor-pointer transition-all ${selectedStyles.includes(style.name) ? 'ring-2 ring-purple-600 bg-purple-50' : 'bg-gray-50 hover:bg-gray-100'}`}
                            >
